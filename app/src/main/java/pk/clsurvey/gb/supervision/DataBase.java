...
//        String[] clus = {""};
    ...
    public void load_HH(String hh_id, String HH_cluster, String hh_status, String hh_result, String En, String location, String time_, String revisit, String hh_Sr){
        SQLiteDatabase db = this.getWritableDatabase();
    ...
    public ArrayList<status> fetch_statuses(String cluster){
        ...
        String selectQuery = "SELECT "+ HH_id+ "," +HH_status + "," +HH_result+"," + timestamp+"," + revisit_time+" FROM " + TABLE_HH +" WHERE " + Cluster_code + "='"+cluster+"' order by  CAST("+HH_Sr+" as INT)";
        ...
        if (cursor.moveToFirst()) {
            do {
                ...
                contact.setRevisit(cursor.getString(4));
    ...
    public void update_status_hh(String hh_id, String Cluster,  String En, String location, String revisit_, String status, String hh_result, String time_, String sync_){
        SQLiteDatabase db = this.getWritableDatabase();
        ContentValues values = new ContentValues();
...
//        db.update(TABLE_HH, values, HH_id + " = "+hh_id + " AND cluster_code ="+Cluster, null);
    ...
    public ArrayList<status_HH_model> syncing_data(String date)
    {
        ...
        if (cursor.moveToFirst()) {
            do {
                ...
                contact.setHh_id(cursor.getString(0));//ok
                contact.setCluster(cursor.getString(1));//ok
                contact.setHH_Result(cursor.getString(2));//ok
                contact.setHH_status(cursor.getString(3));//ok
                contact.setEnum_code(cursor.getString(4));//4revisit
                contact.setTimestamp(cursor.getString(5));//5enum
                contact.setLoc(cursor.getString(6));//6timestamp
                contact.setRevist_time(cursor.getString(7));//7loc
                // Adding contact to list
                status_List.add(contact);
            } while (cursor.moveToNext());
        }
    ...
    {
        String selectQuery = "SELECT * FROM " + TABLE_HH +" WHERE "+HH_status+"  like 'Visited' order by "+Cluster_code;
        ArrayList<status_HH_model> status_List = new ArrayList<status_HH_model>();
        SQLiteDatabase db = this.getWritableDatabase();
        Cursor cursor = db.rawQuery(selectQuery, null);
        ...
        if (cursor.moveToFirst()) {
            do {
                ...
                contact.setTimestamp(cursor.getString(6));//5enum
                contact.setEnum_code(cursor.getString(5));//4revist
                contact.setLoc(cursor.getString(7));//6time
                contact.setRevist_time(cursor.getString(4));//7loc
                // Adding contact to list
                status_List.add(contact);
            } while (cursor.moveToNext());
        }
    ...
    public String cluster_count(String cluster_selected) {
        String selectQuery = "SELECT COUNT("+HH_id+") FROM " + TABLE_HH +" where "+Cluster_code+" = '"+cluster_selected+" " ;
        SQLiteDatabase db = this.getWritableDatabase();
        Cursor cursor = db.rawQuery(selectQuery, null);
        cursor.moveToFirst();

...
//                    if(vwParentRow.getBackground().is)
                    db.update_status_hh(hh_id, cluster_selected,"", "",  "NULL", "Not Visited","", "NULL", "1");
                    setview(cluster_selected);
                }
            });
                ...
                if(timePicker.isShown()) {
                   ...
                   // String[] ccode = cluster_selected.split("-");
                    ...
                    if (loc.contains("one"))
                        loc = "0-0";
                    String timeStamp = new SimpleDateFormat("yyyy-MM-dd HH:MM:SS").format(new java.util.Date());
                    db.update_status_hh(hh_id, cluster_selected, "", loc, date + "-" + time_re, "Revisit","", timeStamp,"1");
...
//                child.setHighlightColor(45621);
    ...
    {
        final String[] Result = {"Successfully Interviewed", "No eligible Child", "refusal or unavailable"};
        ...
        builder.setTitle("Responce to "+Enm);
...
//                });
    ...
    public void update_status(String enumerator, String HH_id_, String status) {
        ...
        if(loc.contains("Non")){
           ...
           // String[] ccode =cluster_selected.split("-");
          //  Log.i("qqqqq1", ccode[1]);
            db.update_status_hh(HH_id_,cluster_selected, enumerator,loc , "", "visited", status, timeStamp,"1");
//            sync_date_pushed();
//            finish();
...
//                            Log_entries.d("Error.Respon_send_date", error.toString());
                ...
                protected Map<String, String> getParams()
                {
                    ...
                    params.put("supervisor", code);
                    if(status_hh_model.getRevist_time() == null)
                    {params.put("revist","");}
                    else {
    ...
    private String emptycheck(String revist_time) {
        if(revist_time == null)
            return "";
        else {
            return revist_time;
